...
export default function App() {
  ...
  const [registrations, setRegistrations] = useState(() => JSON.parse(localStorage.getItem('registrations_v1') || '[]'));

  useEffect(() => { localStorage.setItem('registrations_v1', JSON.stringify(registrations)); }, [registrations]);

  const filtered = useMemo(() => {
    let list = events.filter(e => {
      const q = query.trim().toLowerCase();
      if (q) return e.title.toLowerCase().includes(q) || e.description.toLowerCase().includes(q) || e.location.toLowerCase().includes(q);
      return true;
    });
  ...
  const handleSubmit = (ev) => {
    ...
    if (Object.keys(e).length === 0 && selectedEvent) {
      const reg = { id: Date.now(), eventTitle: selectedEvent.title, ...form, createdAt: new Date().toISOString() };
      ...
      <main className="grid">
        {filtered.map(ev => (
          <div key={ev.id} className="card">
            <h2>{ev.title}</h2>
            <p className="meta">{formatDate(ev.date)} • {ev.location}</p>
            <p>{ev.description}</p>
            <div className="card-actions">
              <span>{ev.price === 0 ? 'Free' : `$${ev.price}`}</span>
              <button onClick={() => openRegister(ev)}>Register</button>
            </div>
          </div>
        ))}
      </main>
